...
public class SQLiteDatabaseProvider extends AbstractDatabaseProvider {
    ...
    protected String getCreateMapsTableSQL() {
        return "CREATE TABLE IF NOT EXISTS mapdb_maps (" +
            ...
            "  metadata TEXT" +
            ");" +
            "CREATE INDEX IF NOT EXISTS idx_maps_plugin_id ON mapdb_maps(plugin_id);" +
            "CREATE INDEX IF NOT EXISTS idx_maps_creator_uuid ON mapdb_maps(creator_uuid);" +
            "CREATE INDEX IF NOT EXISTS idx_maps_group_id ON mapdb_maps(group_id);";
    }

    @Override
    protected String getCreateMapDataTableSQL() {
        return "CREATE TABLE IF NOT EXISTS mapdb_data (" +
    ...
    protected String getCreateGroupsTableSQL() {
        return "CREATE TABLE IF NOT EXISTS mapdb_groups (" +
            "  id INTEGER PRIMARY KEY," +
            "  plugin_id TEXT NOT NULL," +
            "  creator_uuid TEXT," +
            ...
            "  metadata TEXT" +
            ");" +
            "CREATE INDEX IF NOT EXISTS idx_groups_plugin_id ON mapdb_groups(plugin_id);";
    }

    @Override
    protected String getCreateIdTableSQL() {
        return "CREATE TABLE IF NOT EXISTS mapdb_ids (" +
    ...
    protected void initializeIdCounters(Connection conn) throws SQLException {
        String sql = "INSERT OR IGNORE INTO mapdb_ids (id_type, current_value) VALUES (?, ?)";
