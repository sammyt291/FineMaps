...
import com.example.mapdb.api.event.MapCreateEvent;
import com.example.mapdb.api.event.MapDeleteEvent;
import com.example.mapdb.api.event.MapLoadEvent;
 ...
 */
public class MapManager implements MapDBAPI {
    ...
    public MapManager(Plugin plugin, MapDBConfig config, DatabaseProvider database, 
        ...
        this.mapIdKey = new NamespacedKey(plugin, "mapdb_id");
    ...
    public boolean canCreateMaps(Player player) {
        if (player.hasPermission("mapdb.unlimited")) {
            return true;
        }
    ...
    public int getMapLimit(Player player) {
        ...
        for (Map.Entry<String, Integer> entry : config.getPermissions().getGroupLimits().entrySet()) {
            if (player.hasPermission("mapdb.limit." + entry.getKey())) {
