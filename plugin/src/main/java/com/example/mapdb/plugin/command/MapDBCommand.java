package com.example.mapdb.plugin.command;
...
/**
 * Main command handler for MapDB.
 */
public class MapDBCommand implements CommandExecutor, TabCompleter {

    private final MapDBPlugin plugin;
    private final MapManager mapManager;
    private final MapDBConfig config;

    public MapDBCommand(MapDBPlugin plugin) {
        this.plugin = plugin;
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(ChatColor.GOLD + "=== MapDB Commands ===");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb url <url> [width] [height] [dither]" + 
                          ChatColor.GRAY + " - Create map from URL");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb get <mapId>" + 
                          ChatColor.GRAY + " - Get a map item");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb delete <mapId>" + 
                          ChatColor.GRAY + " - Delete a map");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb list [pluginId]" + 
                          ChatColor.GRAY + " - List maps");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb info <mapId>" + 
                          ChatColor.GRAY + " - Show map info");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb stats" + 
                          ChatColor.GRAY + " - Show statistics");
        sender.sendMessage(ChatColor.YELLOW + "/mapdb reload" + 
    ...
    private boolean handleCreate(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission("mapdb.create")) {
        ...
        mapManager.createMap("mapdb", pixels).thenAccept(map -> {
    ...
    private boolean handleFromUrl(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission("mapdb.url")) {
        ...
        if (args.length < 2) {
            player.sendMessage(ChatColor.RED + "Usage: /mapdb url <url> [width] [height] [dither]");
            return true;
        }
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (finalWidth == 1 && finalHeight == 1) {
                    // Single map
                    mapManager.createMapFromImage("mapdb", image, finalDither).thenAccept(map -> {
                        plugin.getServer().getScheduler().runTask(plugin, () -> {
                            mapManager.giveMapToPlayer(player, map.getId());
                    ...
                    // Multi-block map
                    mapManager.createMultiBlockMap("mapdb", image, finalWidth, finalHeight, finalDither)
                        .thenAccept(multiMap -> {
                            plugin.getServer().getScheduler().runTask(plugin, () -> {
    ...
    private boolean handleGive(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission("mapdb.get")) {
            player.sendMessage(ChatColor.RED + "You don't have permission to get maps.");
            return true;
        }

        if (args.length < 2) {
            player.sendMessage(ChatColor.RED + "Usage: /mapdb get <mapId>");
            return true;
        }
    ...
    private boolean handleDelete(CommandSender sender, String[] args) {
        if (!sender.hasPermission("mapdb.delete")) {
            sender.sendMessage(ChatColor.RED + "You don't have permission to delete maps.");
            return true;
        }

        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "Usage: /mapdb delete <mapId>");
            return true;
        }
    ...
    private boolean handleList(CommandSender sender, String[] args) {
        if (!sender.hasPermission("mapdb.list")) {
            sender.sendMessage(ChatColor.RED + "You don't have permission to list maps.");
            return true;
        }

        String pluginId = args.length >= 2 ? args[1] : "mapdb";
    ...
    private boolean handleInfo(CommandSender sender, String[] args) {
        if (!sender.hasPermission("mapdb.info")) {
            sender.sendMessage(ChatColor.RED + "You don't have permission to view map info.");
            return true;
        }

        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "Usage: /mapdb info <mapId>");
            return true;
        }
    ...
    private boolean handleReload(CommandSender sender) {
        if (!sender.hasPermission("mapdb.admin")) {
    ...
    private boolean handleStats(CommandSender sender) {
        ...
        mapManager.getMapCount("mapdb").thenAccept(count -> {
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                sender.sendMessage(ChatColor.GOLD + "=== MapDB Stats ===");
